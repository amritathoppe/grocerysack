...
<BODY>
	...
	<?php
	 ...
	 echo'<li><a href="#">'.$row1[0].'</a></li>';
		   ...
		   <form class="navbar-form"  action="search_allgrocery.php" method="post">
...
<div class="txt-heading">Shopping Cart <a id="btnEmpty" href="allgrocery.php?action=empty">Empty Cart</a></div> 
<?php
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		?>
				...
				<td><a href="allgrocery.php?action=remove&product_code=<?php echo $item["product_code"]; ?>" class="btnRemoveAction">Remove Item</a></td>
				</tr>
				<?php
...
<div id="product-grid">
	...
	<?php
	$product_array = $db_handle->runQuery("SELECT * FROM products where product_cater='grocery' ORDER BY product_id ASC");
	if (!empty($product_array)) { 
		foreach($product_array as $key=>$value){
	?>
		<div class="product-item">
			<form method="post" action="allgrocery.php?action=add&product_code=<?php echo $product_array[$key]["product_code"]; ?>">
